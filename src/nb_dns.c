...
/* Forwards */
static int _nb_dns_mkquery(struct nb_dns_info*, const char*, int, int, void*, char*);
...
static const char* sa_ntop(struct sockaddr* sa, char* buf, int len)
	...
	if ( sa->sa_family == AF_INET )
		return inet_ntop(sa->sa_family, &(((struct sockaddr_in*)sa)->sin_addr), buf, len);
	else
		return inet_ntop(sa->sa_family, &(((struct sockaddr_in6*)sa)->sin6_addr), buf, len);
	}

struct nb_dns_info* nb_dns_init(char* errstr)
	{
	...
	if ( nd == NULL )
		{
		snprintf(errstr, NB_DNS_ERRSIZE, "nb_dns_init: malloc(): %s", my_strerror(errno));
		return (NULL);
		}
		...
		if ( config_file )
			...
			while ( fgets(line, sizeof(line), config_file) )
				...
				if ( inet_pton(AF_INET, ns, &a.sin_addr) == 1 )
					...
					if ( nd->s < 0 )
						{
						snprintf(errstr, NB_DNS_ERRSIZE, "socket(): %s", my_strerror(errno));
						fclose(config_file);
						free(nd);
						return (NULL);
						}
					...
					if ( connect(nd->s, (struct sockaddr*)&nd->server,
						...
						snprintf(errstr, NB_DNS_ERRSIZE, "connect(%s): %s", s, my_strerror(errno));
						fclose(config_file);
						close(nd->s);
						free(nd);
						return (NULL);
			...
			fclose(config_file);
			snprintf(errstr, NB_DNS_ERRSIZE, "no valid nameserver found in %s", config_file_path);
			free(nd);
			return (NULL);
			}
	...
	for ( i = 0; i < _res.nscount; ++i )
		...
		if ( connect(nd->s, (struct sockaddr*)&nd->server,
		             nd->server.ss_family == AF_INET ? sizeof(struct sockaddr_in)
		                                             : sizeof(struct sockaddr_in6)) < 0 )
			{
			char s[INET6_ADDRSTRLEN];
			sa_ntop((struct sockaddr*)&nd->server, s, INET6_ADDRSTRLEN);
...
struct nb_dns_info* nb_dns_init2(char* errstr, struct sockaddr* sa)
	{
	register struct nb_dns_info* nd;
...
void nb_dns_finish(struct nb_dns_info* nd)
	{
...
int nb_dns_fd(struct nb_dns_info* nd)
	{

	return (nd->s);
	}

static int _nb_dns_cmpsockaddr(register struct sockaddr* sa1, register struct sockaddr* sa2,
	...
	switch ( sa1->sa_family )
		{
			...
			if ( memcmp(&sin6a->sin6_addr, &sin6b->sin6_addr, sizeof(sin6a->sin6_addr)) != 0 )
				{
...
static int _nb_dns_mkquery(register struct nb_dns_info* nd, register const char* name,
	...
	if ( ne == NULL )
		{
		snprintf(errstr, NB_DNS_ERRSIZE, "malloc(): %s", my_strerror(errno));
		return (-1);
		}
	...
	switch ( atype )
		{
			...
			default:
			snprintf(errstr, NB_DNS_ERRSIZE, "_nb_dns_mkquery: bad family %d", atype);
			free(ne);
			return (-1);
		}

	/* Build the request */
	n = res_mkquery(ns_o_query, /* op code (query) */
	                name, /* domain name */
	...
	if ( send(nd->s, (char*)msg, n, 0) != n )
		{
		snprintf(errstr, NB_DNS_ERRSIZE, "send(): %s", my_strerror(errno));
		free(ne);
		return (-1);
		}
...
int nb_dns_host_request(register struct nb_dns_info* nd, register const char* name,
...
int nb_dns_host_request2(register struct nb_dns_info* nd, register const char* name,
                         register int af, register int qtype, register void* cookie,
                         register char* errstr)
	{
	if ( qtype != 16 )
		{
...
int nb_dns_addr_request(register struct nb_dns_info* nd, nb_uint32_t addr, register void* cookie,
                        register char* errstr)
	{

	return (nb_dns_addr_request2(nd, (char*)&addr, AF_INET, cookie, errstr));
	}

int nb_dns_addr_request2(register struct nb_dns_info* nd, char* addrp, register int af,
                         register void* cookie, register char* errstr)
	{
#ifdef AF_INET6
	register char* cp;
	...
	switch ( af )
		{
			...
			case AF_INET:
			uaddr = (u_char*)addrp;
			snprintf(name, sizeof(name), "%u.%u.%u.%u.in-addr.arpa", (uaddr[3] & 0xff),
			...
			for ( n = NS_IN6ADDRSZ - 1; n >= 0; --n )
				{
				snprintf(cp, size, "%x.%x.", (uaddr[n] & 0xf), (uaddr[n] >> 4) & 0xf);
				i = strlen(cp);
				size -= i;
				cp += i;
				}
			...
			default:
			snprintf(errstr, NB_DNS_ERRSIZE, "nb_dns_addr_request2(): unknown address family %d",
			         af);
			return (-1);
		}
...
int nb_dns_abort_request(struct nb_dns_info* nd, void* cookie)
	{
...
int nb_dns_activity(struct nb_dns_info* nd, struct nb_dns_result* nr, char* errstr)
	{
	...
	fromlen = sizeof(from);
	msglen = recvfrom(nd->s, (char*)msg, sizeof(msg), 0, (struct sockaddr*)&from, &fromlen);
	if ( msglen <= 0 )
		{
		snprintf(errstr, NB_DNS_ERRSIZE, "recvfrom(): %s", my_strerror(errno));
		return (-1);
		}
	if ( msglen < HFIXEDSZ )
		{
		snprintf(errstr, NB_DNS_ERRSIZE, "recvfrom(): undersized: %d", msglen);
		return (-1);
		}
	if ( ns_initparse((u_char*)msg, msglen, &handle) < 0 )
		{
		snprintf(errstr, NB_DNS_ERRSIZE, "ns_initparse(): %s", my_strerror(errno));
		nr->host_errno = NO_RECOVERY;
		return (-1);
		}

	/* RES_INSECURE1 style check */
	if ( _nb_dns_cmpsockaddr((struct sockaddr*)&nd->server, (struct sockaddr*)&from, errstr) < 0 )
		{
		nr->host_errno = NO_RECOVERY;
		return (-1);
		}
		...
		switch ( atype )
			{
				...
				if ( rdlen != (unsigned int)he->h_length )
					{
					snprintf(errstr, NB_DNS_ERRSIZE, "nb_dns_activity(): bad rdlen %d", (int)rdlen);
					nr->host_errno = NO_RECOVERY;
					return (-1);
					}
				...
				if ( bp + rdlen >= ep )
					{
					snprintf(errstr, NB_DNS_ERRSIZE, "nb_dns_activity(): overflow 1");
					nr->host_errno = NO_RECOVERY;
					return (-1);
					}
				if ( nd->dns_hostent.numaddrs + 1 >= MAXADDRS )
					{
					snprintf(errstr, NB_DNS_ERRSIZE, "nb_dns_activity(): overflow 2");
					nr->host_errno = NO_RECOVERY;
					return (-1);
					}
				...
				*hap++ = bp;
				...
				if ( bp + rdlen >= ep )
					{
					snprintf(errstr, NB_DNS_ERRSIZE, "nb_dns_activity(): overflow 1 for txt");
					nr->host_errno = NO_RECOVERY;
					return (-1);
					}
				...
				case T_PTR:
				n = dn_expand((const u_char*)msg, (const u_char*)msg + msglen, rdata, bp, ep - bp);
				if ( n < 0 )
					{
					/* XXX return -1 here ??? */
					nr->host_errno = NO_RECOVERY;
