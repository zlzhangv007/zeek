...
public:
	...
	T* Remove(const detail::HashKey* key)
...
#pragma GCC diagnostic ignored "-Wdeprecated-declarations"
	...
	T* NextEntryNonConst(detail::HashKey*& h, IterCookie<T>*& c, bool return_hash)
		...
		if ( c->inserted && ! c->inserted->empty() )
			...
			detail::DictEntry<T> e = c->inserted->back();
			if ( return_hash )
...
public:
	...
	T* Insert(const char* key, T* val)
		{
		detail::HashKey h(key);
		return Dictionary<T>::Insert(&h, val);
		}
	T* Insert(detail::HashKey* key, T* val)
		{ return Dictionary<T>::Insert(key, val); }
	T* NthEntry(int n) const
...
#pragma GCC diagnostic pop
	T* RemoveEntry(const detail::HashKey* key)
		{ return Dictionary<T>::Remove(key->Key(), key->Size(), key->Hash()); }
	T* RemoveEntry(const detail::HashKey& key)
		{ return Dictionary<T>::Remove(key.Key(), key.Size(), key.Hash()); }
};
...
template<typename T> using Dictionary [[deprecated("Remove in v4.1. Use zeek::Dictionary instead.")]] = zeek::Dictionary<T>;
